...
with open(file_to_save, "w") as txt_file:
    ...
    for counties_name in counties_votes:
        ...
        c_votes = counties_votes.get(counties_name)
        # 6c: Calculate the percentage of votes for the county.
        county_vote_percentage = float(c_votes) / float(total_votes) * 100
        county_results = (
            f"{counties_name}: {county_vote_percentage:.1f}% ({c_votes:,})\n"
        )
        ...
        if (c_votes > highest_county_turnout):
            highest_county_turnout = c_votes
            largest_county = counties_name

    # 7: Print the county with the largest turnout to the terminal.
    print(f"-------------------------\n"
    f"Largest County Turnout: {largest_county} County\n"
    f"-------------------------\n")
